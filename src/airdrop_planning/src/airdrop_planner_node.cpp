...
//            heading DIKUNCI), trigger pelepasan payload via service MAVROS
//            (DO_SET_SERVO), deteksi missed target -> replan/abort
...
public:
  ...
  : Node("airdrop_planner")
  {
    ...
    cli_command_ = create_client<mavros_msgs::srv::CommandLong>(
      get_parameter("topics.command_service").as_string());
...
private:
  ...
  void declareParams()
  {
    ...
    // mekanisme drop (DO_SET_SERVO)
    declare_parameter("drop.servo_channel_bay", std::vector<int64_t>{9, 10});
    declare_parameter("drop.pwm_release", 1900);

    // topik & service MAVROS (mudah di-remap)
    ...
    declare_parameter("topics.command_service", std::string("mavros/cmd/command"));
  }
    ...
    std::shared_ptr<const TargetAirdrop::Goal> goal)
  {
    ...
    if (goal->bay_index >= servo_channels_.size()) {
  ...
  void releasePayload()
  {
    ...
    req->confirmation = 0;
    req->param1 = static_cast<float>(servo_channels_[active_bay_]);
    req->param2 = static_cast<float>(pwm_release_);

    if (!cli_command_->service_is_ready()) {
      RCLCPP_ERROR(get_logger(), "Service %s tidak tersedia!",
    ...
    cli_command_->async_send_request(req,
      [this](rclcpp::Client<mavros_msgs::srv::CommandLong>::SharedFuture fut) {
        ...
        RCLCPP_INFO(get_logger(), "DO_SET_SERVO ack: success=%d result=%u",
          res->success, res->result);
      });

    RCLCPP_INFO(get_logger(), "RELEASE! bay=%u servo=%ld pwm=%d (miss %.1f m)",
      active_bay_, servo_channels_[active_bay_], pwm_release_, release_miss_);
  }
  ...
  void finish(rclcpp_action::ResultCode code, bool success, const std::string & msg)
  {
    if (goal_handle_) {
      ...
      result->message = msg;
      result->release_miss_m = release_miss_;
      result->replans = static_cast<uint8_t>(replan_count_);
      switch (code) {
  ...
  std::vector<int64_t> servo_channels_{9, 10};
